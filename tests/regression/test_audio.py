...
from conduit.sync import Model, Response, Conduit
...
def test_audiomessage_from_file():
    ...
    audio_msg = AudioMessage.from_audio_file(
        role="user", text_content="Transcribe this", audio_file=sample_audio_file
    )
...
def test_conduit_with_audiomessage():
    """Test conduit with AudioMessage"""
    ...
    response = Conduit.run(messages=[audio_msg])
    assert isinstance(response, Response)
    assert isinstance(response.content, str)
...
def test_audiomessage_to_openai():
    ...
    audio_msg = AudioMessage.from_audio_file(
        role="user", text_content="Test transcription", audio_file=sample_audio_file
    )
