...
**Next concrete steps**:
1. Embedding model + source-artifact swap per `siphon-server/dev/retrieval.md` Phases R2-R4: nomic-embed-text-v2 (768d, 8K seq), `get_embed_texts` returns description only, drop+rebuild HNSW, NULL existing embeddings, then re-embed all rows. Gated on description regeneration for stored rows.
2. Re-enrich stored Siphon rows so their descriptions get regenerated under the new HyDE design (existing rows in the DB still hold legacy descriptions).
...
- [x] HyDE description for article shipped (commit `83fbb23`, deployed 2026-06-04). Sequential summary -> description, gpt-oss/bywater one-shot.
- [x] RoutingSummarizer summary path shipped for 9 remaining enrichers (commit `2b39ca9`).
- [x] HyDE description rollout to 9 remaining enrichers shipped (Phase R5). Per-source `description_guideline.jinja2` files authored; all enrichers now run summary -> description -> title sequentially with HyDE-shaped descriptions.

**Open (in priority order)**:
- [ ] Embedding model + source-artifact swap (`retrieval.md` Phases R2-R4): nomic-embed-text-v2, 768d, FTS index on description+summary, RRF query layer. Re-embed all stored rows.
- [ ] Re-enrich stored Siphon rows so their descriptions get regenerated under the new HyDE design.
...
- [ ] Close the `RoutingSummarizer`-in-eval-matrix gap: `_config_id` JSON-serialization currently can't handle Pydantic-model + `type` fields, so the router can't be A/B-tested as one matrix row. Fix via routing-name registry or custom encoder.
