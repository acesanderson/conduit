...
class ChatEngine:
    ...
    def __init__(self, params: GenerationParams | None = None, options: ConduitOptions | None = None):
        self.conversation = Conversation()
        self.params = params or GenerationParams.defaults("claude-3-sonnet") # Default if not provided
    ...
    def _parse_command_args(self, command_string: str) -> tuple[str, list[str]]:
        ...
        for registered_cmd in sorted(self.commands.keys(), key=len, reverse=True):
            if command_string.startswith(registered_cmd[1:]): # Compare without leading slash
                cmd_name = registered_cmd
                remaining_args_string = command_string[len(registered_cmd[1:]):].strip()
                break
    ...
    async def handle_query(self, user_input: str) -> str | None:
        ...
        self.conversation = await Engine.run(self.conversation, params=self.params, options=self.options)

        if self.conversation.messages and self.conversation.messages[-1].role == Role.ASSISTANT:
        ...
        if handler:
            ...
            if cmd_name == "/set model":
                if args:
                    ...
                    return await handler(app, None) # Or raise an error, or return a usage string
            else:
